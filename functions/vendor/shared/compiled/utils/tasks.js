...
export function taskDueAt(dateKey, time, timeZone) {
    if (time)
...
export function isTaskOverdue(task, now = new Date(), timeZone = 'UTC') {
    if (task.isDeleted || task.status === 'DONE' || task.status === 'CANCELLED' || !task.dueDate)
        return false;
    if (!task.dueTime)
        return task.dueDate < dateKeyInTimeZone(now, timeZone);
    return Boolean(task.dueAt && dateFrom(task.dueAt).getTime() < now.getTime());
}
export function isTaskDueToday(task, now = new Date(), timeZone = 'UTC') {
...
export function nextTaskRecurrenceDate(currentDate, recurrence, nextOccurrenceNumber) {
    if (recurrence.endType === 'AFTER_OCCURRENCES' &&
        nextOccurrenceNumber > (recurrence.maxOccurrences ?? 0))
        return null;
    let next;
    if (recurrence.frequency === 'DAILY')
        next = addDays(currentDate, recurrence.interval);
        ...
        next = addDays(currentDate, 7 * recurrence.interval);
    else if (recurrence.frequency === 'MONTHLY')
        next = addMonths(currentDate, recurrence.interval);
    ...
    if (recurrence.endType === 'UNTIL_DATE' && next > (recurrence.untilDateKey ?? ''))
        return null;
    return next;
}
export function taskReminderAt(dueAt, offsetMinutes) {
    return offsetMinutes == null ? null : new Date(dueAt.getTime() - offsetMinutes * 60_000);
}
export function taskReminderStatesAfterRestore(task, now = new Date()) {
    const active = task.status !== 'DONE' && task.status !== 'CANCELLED';
    const reminderPending = active && task.reminderAt && dateFrom(task.reminderAt).getTime() > now.getTime();
    return {
