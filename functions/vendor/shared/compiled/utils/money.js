const MINOR_DIGITS = { JPY: 0, KRW: 0, BHD: 3, KWD: 3 };
export function currencyMinorDigits(currency) {
    return MINOR_DIGITS[currency.toUpperCase()] ?? 2;
}
export function parseMoneyToMinor(input, currency = 'EUR') {
    ...
    if (fraction.length > digits)
        throw new Error(`Amount supports at most ${digits} decimal places`);
...
export function formatMoney(amountMinor, currency = 'EUR', locale = 'en-IE') {
    if (!Number.isSafeInteger(amountMinor))
        throw new Error('Money must be a safe integer');
    const digits = currencyMinorDigits(currency);
...
export function addMinor(...amounts) {
    ...
    if (!Number.isSafeInteger(total))
        throw new Error('Money calculation overflow');
    return total;
}
